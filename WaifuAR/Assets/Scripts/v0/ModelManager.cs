...
namespace ARKitPlaneSaver
{
    ...
    public class ModelManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            this.controller = GameObject.FindGameObjectWithTag("Manager");
            if (controller == null)
            {
                Debug.Log("Manager(Canvas) was not found!");
            }
        }
        ...
        void Update()
        {
            ...
            // check if atNewMap
            //if (!controller.GetComponent<ARKitPlaneSaver>().LocalizedFirstTime && !controller.GetComponent<ARKitPlaneSaver>().AtNewMap) return;
            if (!controller.GetComponent<ARKitPlaneSaver>().AtNewMap) return;
